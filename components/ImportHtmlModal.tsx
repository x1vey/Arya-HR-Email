...
export function ImportHtmlModal({ open, onClose, onImported }: ImportHtmlModalProps) {
  ...
  const handleImport = async () => {
    ...
    const provider: AiProvider = saved === "groq" ? "groq" : saved === "openrouter" ? "openrouter" : "gemini";
      ...
      >
        ...
        <div className="flex flex-col gap-3 px-6 py-5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-ink">HTML source</span>
            <button
              onClick={handlePaste}
              className="flex items-center gap-1 rounded-md border border-brand-pale px-2 py-1 text-[11px] font-medium text-muted transition hover:border-brand/40 hover:text-ink"
            >
          ...
          <textarea
            ...
            rows={12}
            disabled={loading}
            className="w-full resize-none rounded-xl border border-brand-pale bg-canvas px-4 py-3 font-mono text-xs leading-relaxed text-ink placeholder:text-muted/60 focus:border-brand focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand/20 disabled:opacity-60"
          />
